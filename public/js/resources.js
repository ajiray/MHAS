...
function displayResources(resources) {
    ...
    var resourcesContainer = $('<div class="flex flex-wrap justify-around mt-8"></div>');
    ...
    resources.forEach(function (resource) {
        ...
        var resourceItem = $('<div class="border border-solid border-gray-300 m-4 p-6 rounded-lg shadow-md text-center w-64 h-96 flex flex-col"></div>');
        ...
        if ((coverPhotoPath && (resource.category === 'pdf' || resource.category === 'ebook'))) {
            ...
            var coverPhoto = $('<img class="w-32 h-32 mb-4 object-cover" src="' + coverPhotoPath + '" alt="Cover Photo">');
        ...
        if (resource.category === 'video' && resource.youtube_link) {
            // Assuming you have a container div with the id 'videoContainer'
            var videoContainer = $('<div class="mt-auto mb-4"></div>');

            // Extract the YouTube video ID from the URL
            var videoId = resource.youtube_link;

            // Create the iframe element
            var iframe = $('<iframe width="100%" height="180" src="' + videoId + '" frameborder="0" allowfullscreen></iframe>');
        ...
        if (resource.category === 'infographic' && resourcePath) {
            var infographicImage = $('<img class="w-32 h-32 mb-4 object-cover" src="' + resourcePath + '" alt="Infographic">');
            resourceItem.append(infographicImage);
        }

        // Add margin to create a gap between resource items
        resourceItem.css('margin', '0 10px');
...
function toggleInputs() {
    ...
    var youtubeLinkInput = document.getElementById('youtube_link');
